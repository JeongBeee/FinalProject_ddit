					...
					xhr.onreadystatechange = () => {
						if (xhr.readyState == 4 && xhr.status == 200) {
							...
							for (let i = 0; i < rslt.length; i++) {
								tblStr += `<button type="button" class="btn btn-outline-primary date" style="width:115px;"  onclick="showChart('\${rslt[i].detailRegDate}','\${patCode}','\${rslt[i].treatNo}')">\${rslt[i].detailRegDate}</button><br>`;
							}
							tblStr += "</div>"
